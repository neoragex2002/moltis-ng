...
> - 对外字段命名：统一 `camelCase`（例如 `sessionId` / `chanChatKey` / `chanAccountKey` / `chanReplyTarget`）
> - 文中出现的 `session_key/sessionKey/account_id/account_handle/...` 仅作为历史实现/证据（As-is）引用，不代表应继续使用的对外/协议命名
...
- 目标：按 `sessionId` 做 prompt caching 分桶（可选 hash），并把 bucket id 写入官方字段 `prompt_cache_key`。
   - 配置：`[providers.openai-responses.prompt_cache]`（该块存在且 enabled=true 时才会发送 `prompt_cache_key`）。
- 关键行为：`bucket_hash = "auto"` 按 `sessionId` 的 UTF-8 字节长度阈值（>64）决定是否 hash；短 key 不 hash。
- 补强（可靠性）：当启用 prompt_cache 但本次请求缺失 `sessionId`（legacy 实现字段名可能仍叫 `session_key`）时，仍会发送一个确定性的 fallback `prompt_cache_key`（`moltis:<provider>:<model>:no-session`，同样受 bucket_hash 影响），避免“必须提供 bucket key”的 Responses 网关直接拒绝请求。
   - 证据：
...
3) **Issue 3（prompt cache 显式分桶）**
   - 已完成（见 Executive Summary）。如需开启：新增 `[providers.openai-responses.prompt_cache]`；分桶按 `sessionId`；`bucket_hash=auto`（按 UTF-8 字节长度阈值 64 决定是否 hash）。
...
- 优先使用 `sessionId`（来自 gateway 显式传入的 request context；legacy 字段名可能仍叫 `session_key`）。
- 若本次请求缺失 `sessionId`，则使用确定性的 fallback：`moltis:<provider>:<model>:no-session`（同样受 bucket_hash 影响）。
...
- **用途**：当 `sessionId` 可能过长时，自动把它转换成固定长度的 bucket id（64 hex）。
- 默认规则（你已确认）：
- 长度判定口径：按 **UTF-8 字节长度**（legacy 变量名：`session_key.as_bytes().len()`）。
- 若 `sessionId` UTF-8 字节长度 `<= 64`：默认不 hash（bucket_id=`sessionId`）
- 若 `sessionId` UTF-8 字节长度 `> 64`：默认 hash（bucket_id=hash64hex(`sessionId`))
- 覆盖规则（仍保持参数收敛）：
...
- 当 `bucket_hash=auto` 且 `sessionId` UTF-8 字节长度 `<=64` 时，`prompt_cache_key` 会直接等于 `sessionId`。
- 旧实现中 `session_key`（实际更接近 `chanChatKey`）可能包含渠道标识（例如 Telegram 的 bot/chat/thread）。新口径下 `sessionId` 不应包含任何渠道标识；如需“永不外发可读标识”，仍可强制 `bucket_hash=true`（始终用 64 hex）。
...
- 默认：`bucket_hash=auto`（按 `sessionId` 长度决定）。
- 可观测性：每次请求至少能看到 `prompt_cache_enabled/bucket_hash/cached_tokens`（日志或 debug 面板）。

#### bucket_id 生成规则（你要求给出具体示例供确认）

本小节讨论的 prompt-cache bucket 主称呼是 **`sessionId`**（权威口径：`docs/src/concepts-and-ids.md`）。

legacy 说明：历史实现中常把 `session_key` 作为“会话唯一键”到处传递；其中一部分值实际上更接近 **`chanChatKey`**（确定性渠道对话坐标）。本小节保留 legacy 证据，但正文以 `sessionId/chanChatKey` 作为主称呼。

As-is（legacy）实现中，Gateway 层 `session_key` 的真实格式在仓库内已有证据（明确示例）：
...
- Telegram DM（legacy `session_key`；语义更接近 `chanChatKey`）：`telegram:{account_id}:dm:{peer_id}`
  - 证据：`crates/telegram/src/handlers.rs:1478`
- Telegram Group/Channel（legacy `session_key`；语义更接近 `chanChatKey`）：`telegram:{account_id}:group:{group_id}`
  - 证据：`crates/telegram/src/handlers.rs:1481`

另外还有一种 channel 默认 key 格式（legacy `session_key`；不含 dm/group 细分）：

- `telegram:{account_id}:{chat_id}`
  - 证据：`crates/gateway/src/channel_events.rs:1331`

我们建议 `bucket_id` 生成规则如下（默认不加 hash）：
...
注：新口径下 `prompt_cache_key` 不应直接等于 `chanChatKey`（例如 `telegram:...`）；渠道坐标属于 `chanChatKey`，而 prompt-cache bucket 属于 `sessionId`。

如果你确认要在 session 基础上加 hash（`bucket_hash=true`），则：

- `bucket_id = hash64hex(sessionId)`
  - 输出形式：固定 64 个小写 hex（长度可控）
  - 算法（已确认）：`hash64hex = blake3(sessionId)` 的 hex 编码（32 bytes → 64 hex），对同一个 `sessionId` 必须稳定、跨进程一致。

默认策略（你已确认）：`bucket_hash=auto`（len<=64 不 hash；len>64 hash）。

#### 实施 seam（已确认）：`sessionId` 必须显式传递

为避免从 messages 反推导致不可靠，本方案要求：gateway 在调用 `openai-responses` provider 构造请求体时，**显式传入 `sessionId`**（作为 `prompt_cache_key` 的输入源；legacy 字段名可能仍叫 `session_key`）。
...
### 备注：`data_mount` 的语义（已收敛）

- Docker backend 的 `data_mount` 挂载的是 `tools.exec.sandbox.data_mount_source`（bind absolute path / named volume）到固定容器内路径 `/moltis/data`。
  - 该挂载点是为了让 system prompt 可以稳定引用真实可达路径（例如 `/moltis/data/PEOPLE.md`），并避免泄露宿主机绝对路径。
