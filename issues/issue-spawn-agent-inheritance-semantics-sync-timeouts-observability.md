# Issue: `spawn_agent` 子代理问题总览（继承语义：`sessionId`/model/tool_context/tools/hooks；同步阻塞；超时/断流可观测性）
...
- 子代理 prompt assembly：统一走 canonical v1 builder（单条 `ChatMessage::system`）：`crates/tools/src/spawn_agent.rs:235`、`crates/agents/src/runner.rs:664`

**已完成（已核实）**
- [x] 子代理继承父 run 的关键 `tool_context`（`_sessionId/_chanChatKey/_connId/_acceptLanguage/_sandbox`）：`crates/tools/src/spawn_agent.rs:181`–`crates/tools/src/spawn_agent.rs:218`
...
- **子代理 `sessionId`**：子代理对上游 LLM 请求使用的会话桶标识（用于 prompt-cache bucket、hooks 归属等）。
  - 注：在 typed messages 路径中，该语义对应 `moltis_agents::model::LlmRequestContext.session_id`。
- **model（tool 参数）**：`spawn_agent` 的可选参数，用于指定子代理使用的 model id（注册表中的 key）。
- **default_provider（spawn_agent 内部）**：若 tool 参数未指定 model，当前实现使用 gateway 启动时注入的 `default_provider`，并非“父 run 的当前 provider/model”。
...
- [ ] 子代理的 provider/model 默认行为必须与文案一致：
  ...
  - 当前实现：使用 gateway 启动时注入的 `default_provider`（可能与父 run 不一致）
...
- 子代理复用父 `sessionId`（对 LLM request context / prompt-cache bucket）：
  - gateway 注入：`crates/gateway/src/chat.rs:4725`（`tool_context["_sessionId"]=...`）
  - spawn_agent 透传：`crates/tools/src/spawn_agent.rs:183`–`crates/tools/src/spawn_agent.rs:218`（把 `"_sessionId"` 写入子代理 `tool_context`）
  - runner 使用：`crates/agents/src/runner.rs:803`–`crates/agents/src/runner.rs:813`（从 `tool_context._sessionId` 构建 `LlmRequestContext.session_id` 并传入 provider）
  - 结论：**子代理对 provider 的 request context（含 prompt-cache bucket）当前默认继承父 `sessionId`**
...
- `unknown model:` 空字符串问题：
  - `crates/tools/src/spawn_agent.rs:133`：`let model_id = params["model"].as_str();`（`""` 也会是 Some("")）
  - `crates/tools/src/spawn_agent.rs:154`：`anyhow!("unknown model: {id}")` → 当 `id=""` 时日志呈现为 `unknown model:`
- 文案与实现不一致（会误导用户/LLM）：
  - tool schema 描述：“If not specified, uses the parent's current model.”：`crates/tools/src/spawn_agent.rs:115`–`crates/tools/src/spawn_agent.rs:118`
  - 实现却使用 `default_provider`：`crates/tools/src/spawn_agent.rs:150`–`crates/tools/src/spawn_agent.rs:157`
...
- 子代理不继承 hooks / 不 forward 事件 / 不带 history（影响一致性与可观测性）：
  - 子代理 loop：`crates/tools/src/spawn_agent.rs:233`–`crates/tools/src/spawn_agent.rs:243`
...
- E) **tool_context 口径仍需固化**：关键字段已透传，但缺少回归测试/可能仍漏掉未来新增字段，仍有“同一 session 内工具行为漂移”风险。
...
- [DONE] 透传关键 tool_context：`_sandbox/_acceptLanguage/_connId/_sessionId`（以及 `_chanChatKey` 如适用；至少保证与父 run 的工具行为一致）。
...
- [x] 子代理继承关键 tool_context（至少 `_sandbox/_acceptLanguage/_connId/_sessionId`，以及 `_chanChatKey` 如适用）。
- [ ] tool_context 关键字段具备回归测试覆盖（至少覆盖 `_sandbox/_acceptLanguage/_connId/_sessionId/_chanChatKey`）。
...
- [ ] `spawn_agent`：tool_context 透传（断言 `_sandbox/_acceptLanguage/_connId/_sessionId/_chanChatKey` 在子代理工具调用参数中可见；至少覆盖 `_sandbox`）
