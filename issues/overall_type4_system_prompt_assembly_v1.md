...
- 多行段落变量的空白/换行约定（写死，避免“模板作者没意识到的隐式 padding”）：
  - 要么为 `""`（空字符串），要么必须满足：
    1) **不以换行开头**（避免模板作者在前面加换行时出现双重空行）
    2) **不以换行结尾**（避免模板作者在后面加换行时出现双重空行）
    3) 段落内部换行与缩进必须稳定（固定缩进、固定空行数）
- 特例：`voice_reply_suffix_md` 的上游来源 `VOICE_REPLY_SUFFIX` 历史上以 `\n\n` 开头（便于 append），但作为模板变量值时必须做归一化：trim 掉前导换行与末尾换行；由模板作者决定前后空行。
...
- 内容来源：复用现有固定常量 `VOICE_REPLY_SUFFIX` 的文本（以保持与现状一致，避免 voice 模式行为漂移）；作为模板变量值时必须按本节约定做**归一化**（trim 前导/末尾换行；不做隐式 padding）。
  - Evidence（常量定义）：`crates/agents/src/prompt.rs:785`
  - Evidence（变量生成与归一化）：`crates/agents/src/prompt.rs:177`、`crates/agents/src/prompt.rs:271`
- 注意：该变量的存在是为了让“voice 输出约束”完全由用户模板可控地引用（以空字符串实现可选段落），而不是由系统在模板之外强行追加。
  - 实施治理目标：canonical v1 不再由 gateway 硬编码追加该段落；应只通过模板变量注入，避免重复注入。
