...
## 术语对齐：`subkey` vs `bucket_key`

本文档在 `dm_scope` / `group_scope` 语义讨论里使用：

- `dm_subkey` / `group_subkey`：表示“适配层把分桶语义实现出来后返回的黑盒结果”

在工程实现与运行时表里，也常把这一黑盒结果称为：

- `bucket_key`

两者的关系可以理解成：

- `subkey` 是“接口概念”（黑盒分桶结果）
- `bucket_key` 是“具体编码后的字符串值”（可持久化、可比对、用于查表命中同一个桶）

无论叫哪个名字，核心约束不变：

- 上层只比较相等/不相等，不解析其内部结构
- `session_key` 仍由 core 统一装配（例如：`<agent-prefix> + :<type>: + <bucket_key>`）
