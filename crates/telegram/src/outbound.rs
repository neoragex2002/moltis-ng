...
impl TelegramOutbound {
    ...
    fn group_outbound_plan_dedupe_key(
        ...
        format!(
            "telegram.group.outbound_plan|source:{source_account_handle}|target:{target_account_handle}|chat:{chat_id}|message:{message_id}"
        )
    }

    fn register_group_visible_outbound_author_bindings(
        &self,
        account_handle: &str,
        chat_id: &str,
        ...
        let _ = runtime.register_message_author(
            account_handle,
            chat_id,
            sent_message_ids,
            account_handle,
    ...
    async fn emit_group_visible_outbound_event(
        ...
        self.register_group_visible_outbound_author_bindings(
        ...
        let reply_to_target_account = reply_to_message_id.and_then(|message_id| {
            crate::state::shared_group_runtime(&self.accounts)
                ...
                .message_author(source_account_handle, chat_id, message_id)
        });
        ...
        for (target_account_handle, target_config, target_username, target_bridge) in targets {
            ...
            let dedupe_key = Self::group_outbound_plan_dedupe_key(
                source_account_handle,
                &target_account_handle,
                chat_id,
                sent_message_id,
            );
            let local_msg_ref = crate::state::GroupLocalMessageRef::new(
                source_account_handle,
                chat_id,
                sent_message_id,
            );
            let is_dup = crate::state::shared_group_runtime(&self.accounts)
                .lock()
                .unwrap_or_else(|e| e.into_inner())
                .check_and_insert_action(chat_id, &dedupe_key);
            if is_dup {
                info!(
                    ...
                    local_msg_ref = %local_msg_ref.log_value(),
                    text_len = text.len(),
                    text_preview = %crate::adapter::log_text_preview(text, 240),
            ...
            let inbound = TgInbound {
                ...
                private_source: TgPrivateSource {
                    ...
                    peer: crate::adapter::tgchat_key(chat_id)
    ...
    async fn send_text_inner(
        ...
        let first_local_msg_ref = first_id.as_ref().map(|sent_id| {
            crate::state::GroupLocalMessageRef::new(account_handle, to, &sent_id.0.to_string())
                .log_value()
        });
        info!(
            account_handle,
            chat_id = to,
            reply_to = ?reply_to,
            local_msg_ref = first_local_msg_ref.as_deref(),
            text_len = text.len(),
            text_preview = %crate::adapter::log_text_preview(text, 240),
            chunk_count = chunks.len(),
            silent,
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            let content = if i == last_idx {
                ...
                if combined.len() <= TELEGRAM_MAX_MESSAGE_LEN {
                    ...
                    let first_local_msg_ref = first_id.as_ref().map(|sent_id| {
                        crate::state::GroupLocalMessageRef::new(
                            account_handle,
                            to,
                            &sent_id.0.to_string(),
                        )
...
impl ChannelOutbound for TelegramOutbound {
    ...
    async fn send_text_by_reply_target_ref_with_ref(
        ...
        let reply_target_ref_hash = Self::sha256_hex(reply_target_ref);
        let parsed =
        ...
        let sent = self
            ...
            .await?;
        if let Some(ref message_id) = sent {
            info!(
                event = "telegram.outbound.reply_target_delivery",
                op = "send_text_by_reply_target_ref",
                account_handle = %parsed.target.chan_account_key,
                chat_id = %parsed.target.chat_id,
                reply_to = parsed.target.message_id.as_deref(),
                local_msg_ref = %crate::state::GroupLocalMessageRef::new(
                    &parsed.target.chan_account_key,
                    &parsed.target.chat_id,
                ...
                text_len = text.len(),
                text_preview = %crate::adapter::log_text_preview(text, 240),
                "telegram outbound reply-target delivery linked"
            );
        }
        Ok(sent.map(|id| SentMessageRef {
            message_id: id.0.to_string(),
    ...
    async fn send_text_with_suffix_by_reply_target_ref_with_ref(
        ...
        if let Some(ref message_id) = sent {
            info!(
                event = "telegram.outbound.reply_target_delivery",
                op = "send_text_with_suffix_by_reply_target_ref",
                account_handle = %parsed.target.chan_account_key,
                chat_id = %parsed.target.chat_id,
                reply_to = parsed.target.message_id.as_deref(),
                local_msg_ref = %crate::state::GroupLocalMessageRef::new(
    ...
    async fn send_media_by_reply_target_ref_with_ref(
        ...
        if let Some(ref message_id) = sent {
            info!(
                event = "telegram.outbound.reply_target_delivery",
                op = "send_media_by_reply_target_ref",
                account_handle = %parsed.target.chan_account_key,
                chat_id = %parsed.target.chat_id,
                reply_to = parsed.target.message_id.as_deref(),
                local_msg_ref = %crate::state::GroupLocalMessageRef::new(
                ...
                reply_target_ref_hash = %reply_target_ref_hash,
                caption_len = payload.text.len(),
                text_preview = %crate::adapter::log_text_preview(&payload.text, 240),
                has_media = payload.media.is_some(),
                "telegram outbound reply-target delivery linked"
            );
        }
    ...
    async fn send_location_by_reply_target_ref_with_ref(
        ...
        if let Some(ref message_id) = sent {
            info!(
                event = "telegram.outbound.reply_target_delivery",
                op = "send_location_by_reply_target_ref",
                account_handle = %parsed.target.chan_account_key,
                chat_id = %parsed.target.chat_id,
                reply_to = parsed.target.message_id.as_deref(),
                local_msg_ref = %crate::state::GroupLocalMessageRef::new(
                ...
                longitude,
                has_title = title.is_some(),
                text_preview = %crate::adapter::log_text_preview(title.unwrap_or_default(), 240),
                "telegram outbound reply-target delivery linked"
            );
        }
        Ok(sent.map(|id| SentMessageRef {
...
mod tests {
    ...
    fn capture_json_logs_async<T>(
        ...
        let result = tracing::subscriber::with_default(subscriber, || runtime.block_on(future));
    ...
    #[tokio::test]
    async fn group_visible_outbound_dispatches_without_hidden_root_context() {
        let accounts: AccountStateMap = Arc::new(std::sync::RwLock::new(HashMap::new()));
        let outbound = TelegramOutbound {
            accounts: Arc::clone(&accounts),
        };
    ...
    #[test]
    fn register_group_visible_outbound_author_bindings_propagates_to_all_sent_chunk_ids() {
        let accounts: AccountStateMap = Arc::new(std::sync::RwLock::new(HashMap::new()));
        let outbound = TelegramOutbound {
            accounts: Arc::clone(&accounts),
        };

        outbound.register_group_visible_outbound_author_bindings(
            "telegram:100",
            "-1001",
        ...
        for message_id in ["m101", "m102", "m103"] {
            assert_eq!(
                runtime.message_author("telegram:100", "-1001", message_id),
                Some("telegram:100".to_string())
            );
        }
    }

    #[tokio::test]
    async fn group_visible_outbound_uses_stable_target_order() {
        let accounts: AccountStateMap = Arc::new(std::sync::RwLock::new(HashMap::new()));
        let outbound = TelegramOutbound {
            accounts: Arc::clone(&accounts),
        };
    ...
    #[test]
    fn send_text_by_reply_target_ref_registers_source_author_binding_for_group_send() {
        let (logs, reply_target_ref) = capture_json_logs_async(async {
            ...
            let bot = teloxide::Bot::new("123:ABC")
                .set_api_url(format!("http://{addr}/").parse().unwrap());
            ...
            let mut runtime = shared.lock().unwrap();
            assert_eq!(
                runtime.message_author("telegram:100", "-1001", "501"),
                Some("telegram:100".to_string()),
                "expected sent message to keep source author binding"
            );
        ...
        let expected_reply_target_ref_hash = TelegramOutbound::sha256_hex(&reply_target_ref);
        assert!(
            logs.iter().any(|entry| {
                let Some(fields) = entry.get("fields") else {
                    return false;
                };
                fields.get("event").and_then(|value| value.as_str())
                    == Some("telegram.outbound.reply_target_delivery")
                    && fields
                        .get("reply_target_ref_hash")
                        .and_then(|value| value.as_str())
                        == Some(expected_reply_target_ref_hash.as_str())
                    && fields.get("local_msg_ref").and_then(|value| value.as_str())
                        == Some("telegram:100|-1001|501")
            }),
            "reply-target delivery log must link hash and local_msg_ref"
        );
    }
    ...
    #[tokio::test]
    async fn outbound_plan_does_not_collide_with_existing_inbound_dedupe_key() {
        let accounts: AccountStateMap = Arc::new(std::sync::RwLock::new(HashMap::new()));
        let outbound = TelegramOutbound {
            accounts: Arc::clone(&accounts),
        };
...

        {
            ...
            let mut runtime = shared.lock().unwrap();
            runtime.register_participant("-1001", "telegram:200");
            runtime.check_and_insert_action(
                "-1001",
                "telegram.group.inbound|observer:telegram:200|chat:-1001|message:1955",
            );
        }
    ...
    #[test]
    fn group_visible_outbound_log_contains_local_msg_ref_and_preview() {
        let accounts: AccountStateMap = Arc::new(std::sync::RwLock::new(HashMap::new()));
        let outbound = TelegramOutbound {
            accounts: Arc::clone(&accounts),
        };
        ...
        let (logs, ()) = capture_json_logs_async(async {
            outbound
                .emit_group_visible_outbound_event(
                    "telegram:100",
                    "-1001",
        ...
        let entry = logs
            ...
            .find(|log| {
                log.get("fields")
                    ...
                    == Some("telegram group outbound event handed to gateway")
            })
            .expect("outbound handoff log entry");
        let fields = entry.get("fields").expect("fields");
        assert_eq!(
            fields.get("local_msg_ref").and_then(|value| value.as_str()),
            Some("telegram:100|-1001|1955")
        );
        assert_eq!(
            fields.get("reason_code").and_then(|value| value.as_str()),
            Some("tg_dispatch_promoted_from_record")
        );
        assert_eq!(
            fields.get("decision").and_then(|value| value.as_str()),
            Some("dispatch")
        );
        assert_eq!(
            fields.get("policy").and_then(|value| value.as_str()),
            Some("group_record_dispatch_v3")
        );
        assert!(
            fields
                .get("text_preview")
                .and_then(|value| value.as_str())
                .is_some_and(|preview| preview.contains("@alpha_bot hello from logs"))
        );
    }
