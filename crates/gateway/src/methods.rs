...
impl MethodRegistry {
    ...
    fn register_service_methods(&mut self) {
        ...
        // Workspace (public USER.md)
        self.register(
            "workspace.user.get",
            Box::new(|_ctx| {
                Box::pin(async move {
                    let payload = crate::user::user_get()
        ...
        self.register(
            ...
            Box::new(|ctx| {
                Box::pin(async move {
                    let payload = crate::user::user_update(&ctx.params)
                        .map_err(|e| ErrorShape::new(error_codes::UNAVAILABLE, e.to_string()))?;
                    Ok(payload)
                })
            }),
        ...
        // Workspace (public PEOPLE.md)
        self.register(
            "workspace.people.get",
            Box::new(|_ctx| {
                Box::pin(async move {
                    let payload = crate::people::people_get()
                        .map_err(|e| ErrorShape::new(error_codes::UNAVAILABLE, e.to_string()))?;
                    Ok(payload)
                })
            }),
            ...
            self.register(
                ...
                Box::new(|ctx| {
                    Box::pin(async move {
                        ...
                        let identity = ctx.state.services.onboarding.identity_get().await.map_err(
                            |e| ErrorShape::new(error_codes::UNAVAILABLE, e.to_string()),
                        )?;
                        let user = identity
                            .get("user_name")
                            .and_then(|v| v.as_str())
